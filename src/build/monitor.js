...
async function handleMsg(data) {
    if (cq_1.default.hasImage(data) && this.getCache('params') !== undefined) {
        ...
        if (typeof qr_str === 'undefined')
            this.send('是否已配置腾讯云OCR？图像是否包含清晰二维码？', this.getTargetID());
        else {
...
(async () => {
    ...
    if (process.argv[2] === '--auth') {
        ...
        const userItem = (await (0, prompts_1.default)({ type: 'select', name: 'userItem', message: '选择用户', choices: (0, user_1.getLocalUsers)(), initial: 0 }, PromptsOptions)).userItem;
        if (userItem === -1) {
            const phone = (await (0, prompts_1.default)({ type: 'text', name: 'phone', message: '手机号' }, PromptsOptions)).phone;
            const password = (await (0, prompts_1.default)({ type: 'password', name: 'password', message: '密码' }, PromptsOptions)).password;
